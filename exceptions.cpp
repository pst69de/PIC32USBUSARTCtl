...
is defined in the XC32 User's Guide.  The _weak_ _generic_exception_handler
...
void _general_exception_handler(void)
{
    ...
    _excep_code=(exception_types) (_CP0_GET_CAUSE() & 0x0000007C >> 2);
    ...
    while (1)
    {
        ...
        /* Tip - if the line of source causing the exception can be
         * isolated, C++ has a built in exception class that may be used.
         *
         * The catch/try functions from this class can help find problems.
         *
         * For example:
         *
         *   try
         *   {
         *       int* newarray= new int[<huge number>]; // problem line
         *   }
         *   catch (exception& e)
         *   {
         *       cout << "Exception name: " << e.what() << endl;
         *   }
         *
         * try/catch is an alternative to troubleshooting from the
         * _general_exception_vector and is done by framing the offending
         * line of c source with a try().  Note that this may take inclusion
         * of 'exception', 'iostream' and some heap space which may be added
         * to the xc32-ld in the project properties. */
    }
}
